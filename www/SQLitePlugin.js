(function() {
    ...
    if (this.dbname in this.openDBs) {
      ...
      opensuccesscb = (function(_this) {
        return function(connectionId) {
          ...
          console.log('OPEN database: ' + _this.dbname + ' - OK - connectionId: ' + _this.connectionId);
  ...
  SQLitePlugin.prototype.openDatabaseConnection = function(filename, flags, key, cb, errorCallback) {
    cordova.exec(cb, errorCallback, 'SQLitePlugin', 'openDatabaseConnection', [
      filename,
      flags,
      key
    ])
  ...
  SQLitePlugin.prototype.executeBatch = function (connectionId, batchList, cb) {
    console.log("Using executeBatch in SQLitePlugin.js")
    cordova.exec(cb, null, 'SQLitePlugin', 'executeBatch', [
      connectionId,
      batchList
    ])
  }
  ...
  SQLitePluginTransaction.prototype.run = function() {
    var batchExecutes, handlerFor, i, mycb, errorcb, mycbmap, request, tropts, tx, txFailure, waiting;
    txFailure = null;
    tropts = [];
    ...
    mycb = function(result) {
      console.log("received result from backgroundExecuteSqlBatch: " + JSON.stringify(result))
    ...
    errorcb = function(error) {
      console.log("Error in backgroundExecuteSqlBatch: " + error)
    }
    cordova.exec(mycb, errorcb, "SQLitePlugin", "backgroundExecuteSqlBatch", [
      {
        dbargs: {
          dbname: this.db.dbname,
          connectionId: this.db.connectionId
        },
        executes: tropts
      }
    ]);
